	...
	return (
		<div className='w-full flex justify-center items-center bg-white rounded-full'>
			...
			<div className='w-full rounded-e-full flex justify-end items-center gap-2  p-1 text-gray-400'>
				<button onClick={()=>setOpen(true)} className='flex justify-center items-center text-xl font-bold gap-1'>
	...
	return(
		<Modal title="فیلتر ها" open={open} onClose={onClose}>
			<div className='max-h-[300px] overflow-y-auto pl-3'>
				<ExpertiseFilter show={openFilter} setShow={setOpenFilter} name="expertiseFilter"/>
				<ServicesFilter show={openFilter} setShow={setOpenFilter} name="servicesFilter"/>
				<CityFilter show={openFilter} setShow={setOpenFilter} name="cityFilter"/>
				<GenderFilter show={openFilter} setShow={setOpenFilter} name="genderFilter"/>
			</div>
...
function ExpertiseFilter({show, setShow, name}){
	...
	return(
		<div className={`w-full overflow-hidden ${show === name ? "my-3 rounded-t-lg" : "my-2 rounded-lg"}`}>
			<button onClick={()=>setShow(show === name ? "" : name)} className='w-full flex items-center justify-between p-3 bg-slate-200 text-gray-800 font-semibold'>
				تخصص ها
				<FaAngleDown className={`w-4 h-4 duration-200 ${show === name && "rotate-180"}`} />
			</button>
			<div className={`w-full bg-slate-100 rounded-b-lg px-4 ${show === name ? "max-h-screen" : "max-h-0"} duration-300 ease-in-out`}>
				{!isLoading && 
				<ul>
					{data.map((item, index)=>(
						<li key={index} className='w-full py-3 border-b border-b-slate-200 last:border-b-0'>
...
function ServicesFilter({show, setShow, name}){
	...
	return(
		<div className={`w-full overflow-hidden ${show === name ? "my-3 rounded-t-lg" : "my-2 rounded-lg"}`}>
			...
			<div className={`w-full bg-slate-100 rounded-b-lg px-4 ${show === name ? "max-h-screen" : "max-h-0"} duration-300 ease-in-out`}>
				<ul>
					{Array(4).fill({title:"تست"}).map((item, index)=>(
						<li key={index} className='w-full py-3 border-b border-b-slate-200 last:border-b-0'>
