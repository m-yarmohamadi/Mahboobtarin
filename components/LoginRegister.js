	...
	return (
		<div className={`${isLoading ? 'blur-sm opacity-45' : ''} duration-100`}>
			{token ? (
				...
				<Link
					href='/auth'
					className={`${size === 'sm' ? `w-full flex items-center  justify-center gap-2  text-center ` : `hidden md:flex md:flex-1 md:justify-center md:items-center md:gap-2`}  w-full bg-primary-01 hover:opacity-95 cursor-pointer py-2 px-5 rounded-xl text-white  text-sm`}>
					<IoPerson />
					ورود | ثبت نام
				</Link>
			)}
...
const MenuOptions = forwardRef(function MenuOptionsComponent({ children, open }, ref) {
	if (open)
		return (
			<div
				ref={ref}
				className='absolute top-12 w-60 overflow-hidden left-0 bg-white shadow-2xl rounded-md'>
