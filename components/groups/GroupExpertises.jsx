...
export default function GroupExpertises({ data, searchTerm }) {
    ...
    return (
        <div className="flex flex-col overflow-y-auto">
            {filterCategories(data?.filter((c) => c.parent_id === 0)).map((item) => (
                <GroupTree key={item.id} id={item.id} label={item.name} cateChildren={item?.children_recursive} searchTerm={searchTerm} />
            ))}
        </div>
    );
}

function GroupTree({ id, label, cateChildren, searchTerm }) {
            ...
            {isOpen && filteredChildren.length > 0 && (
                <div className="px-2">
                    {filteredChildren.map((subItem) => (
                        <GroupTree key={subItem.id} id={subItem.id} label={subItem.name} cateChildren={subItem?.children_recursive} searchTerm={searchTerm} />
                    ))}
                </div>
            )}
        </div>
