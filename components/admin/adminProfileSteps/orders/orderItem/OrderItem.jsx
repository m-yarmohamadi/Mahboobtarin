...
export default function OrderItem({ status, data }) {
	...
	const changeStatusHandler = async (statusNum) => {
		changeStatusRequest(
			{ orderid: data.order_id, status: statusNum.toString() },
			{
				onSuccess: (res) => {
		...
		<div className="w-full bg-white rounded-xl p-4">
			...
			{status === '3' && (
				<p className="text-sm font-medium text-slate-700 py-4">
					کاربر گرامی عرضه کننده خدمات به درخواست شما با شرایط ثبت شده تائید اولیه داده
					است. در صورت تمایل تا ۲۴ ساعت پس از دریافت این پیام میتوانید درخواست خود را ثبت
					و تائید نهایی نمایید در غیر اینصورت لطفا سفارش خود را لغو کنید. توجه داشته
					باشید؛ در صورت لغو این ،سفارش شما تا ۷ روز آینده امکان ثبت مجددا این درخواست را
					نخواهید داشت.
				</p>
			)}
				...
				{status === '3' ? (
					<>
						<button
							...
							className="!w-full btn btn--primary !bg-lime-600"
						>
					...
					<button
						onClick={() => router.push(`/${data?.user?.unique_url_id}`)}
						className="!w-full btn btn--outline !text-primary-01 !border-primary-01"
					>
						درخواست مجدد
					</button>
