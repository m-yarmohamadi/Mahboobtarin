	...
	return (
		<form onSubmit={formik.handleSubmit} className="w-full flex flex-col gap-4">
			...
			<div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
				<Input label={'نام فارسی'} formik={formik} name={'title'} required={true} />
				<ExpiredateInput formik={formik} name={'expiredate'} />
				<Input
					label={'قیمت (به تومان)'}
					formik={formik}
					name={'price'}
			...
			<div className="w-full border-t border-t-slate-300 pt-4">
				<div className="w-1/2 gap-3 grid grid-cols-2 ">
					...
					<button className="btn btn--secondary">لغو</button>
				</div>
			</div>
		</form>
	);
...
function CoverImage({ formik }) {
	const { mutateAsync: mutateUploadPhotos, isPending: isUploading } = useMutation({
		mutationFn: updloadAcademyPhotos,
	});
			...
			<div className="w-full sm:max-w-[50%] border border-dashed border-slate-400 rounded-xl overflow-hidden">
				<div className="aspect-w-16 aspect-h-10">
					{formik.values.files.length ? (
						<img
							src={
								formik.values.files[0]?.isEdit
									? formik.values.files[0]?.file
									: URL.createObjectURL(formik.values.files[0].file)
							}
							alt=""
				...
				/>
				<label
					htmlFor="select-cover-course-img"
					className="btn btn--secondary !w-full !rounded-t-none"
				>
					{!isUploading ? 'افزودن تصویر' : 'درحال آپلود...'}
				</label>
			</div>
		...
		<>
			<div className=" bg-white p-4 rounded-xl">
				<div className="flex items-center justify-between border-b border-slate-200 pb-4 mb-4">
					<div className="font-bold text-slate-800">لیست جلسات</div>
					<button
						type="button"
						onClick={() => setAddLesson(true)}
						className="btn btn--outline gap-2 !py-2"
					>
			...
			<div className="w-full flex justify-start items-start">
				{formik?.errors.video_id && formik?.touched.video_id && (
					<p className="error_Message">{formik?.errors.video_id}</p>
				)}
			</div>
		</>
	);
...
function CreateCourse({ formik, onClose }) {
	const { mutateAsync: mutateUploadVideos, isPending: isUploading } = useMutation({
		mutationFn: updloadAcademyVideos,
	});
	...
	return (
		<div className="w-full flex flex-col gap-4">
			<div>
				...
				{formikCourse.values.file ? (
					...
					<>
						...
						<div className="w-full flex justify-start items-start mt-2">
							{formikCourse?.errors.file && formikCourse?.touched.file && (
								<p className="error_Message">{formikCourse?.errors.file}</p>
							)}
						</div>
					</>
				)}
			...
			<Input name={'title'} label={'عنوان'} formik={formikCourse} />
			<div className="flex gap-4">
				<Select
					...
					formik={formikCourse}
					options={[
						{ value: 'kavimo_academy', label: 'kavimo_academy' },
						{ value: 'innternal_academy', label: 'innternal_academy' },
					]}
				/>
				<Select
					name={'free_price'}
