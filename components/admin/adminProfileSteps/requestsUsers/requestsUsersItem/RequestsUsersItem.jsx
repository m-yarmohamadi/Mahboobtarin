...
export default function RequestsUsersItem({ status, data, provinces }) {
	...
	const getProvinceLabel = provinces.filter((p) => p.value === Number(data.user?.province_id))[0]
		?.label;
			...
			<div className="w-full flex flex-col gap-6 sm:grid sm:grid-cols-2 md:grid-cols-3 mb-4">
				<div className="flex items-center gap-2">
					...
					<div className="flex flex-col">
						...
						<span className="text-xs text-slate-600">
							{data?.user?.gender === 'man' ? 'مرد' : 'زن'} |{' '}
							{calculateAge(data?.user?.birthday)}ساله | {getProvinceLabel}
						</span>
					</div>
				</div>
				...
				<div className="flex md:flex-col items-center md:items-start md:justify-center gap-5 md:gap-2">
					<button
						...
						className="flex items-center gap-1 text-sm font-medium text-primary-01"
					>
