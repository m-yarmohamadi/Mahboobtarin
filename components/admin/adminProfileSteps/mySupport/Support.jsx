		...
		<div>
			...
			<div className="w-full">
				<div className="w-full flex justify-between items-center px-2 py-8 ">
					<div className="w-3/5 flex justify-center items-center font-extrabold text-textDefault">
						تیکتهای من
					</div>
					<div className="w-full grid grid-cols-2  lg:grid-cols-4 justify-center items-center gap-4 text-primary-02">
						<button
							onClick={() => setFilter(1)}
							className="focus:outline focus:outline-offset-1 focus:outline-2 focus:outline-primary-01 p-2 rounded-md bg-green-900 text-[#fff] hover:shadow-md dark:shadow-darkMd w-32"
						>
							بسته شده
						</button>
						<button
							onClick={() => setFilter(3)}
							className="focus:outline focus:outline-offset-1 focus:outline-2 focus:outline-primary-01 p-2 rounded-md bg-yellow-400 text-[#fff] hover:shadow-md dark:shadow-darkMd w-32"
						>
							پاسخ داده شده
						</button>
						<button
							onClick={() => setFilter(0)}
							className="focus:outline focus:outline-offset-1 focus:outline-2 focus:outline-primary-01 p-2 rounded-md bg-yellow-900 text-[#fff] hover:shadow-md dark:shadow-darkMd w-32"
						>
							در انتظار پاسخ
						</button>
						<button
							onClick={() => setFilter(2)}
							className="focus:outline focus:outline-offset-1 focus:outline-2 focus:outline-primary-01 p-2 rounded-md bg-red-900 text-[#fff] hover:shadow-md dark:shadow-darkMd w-32"
						>
							باز
						</button>
					</div>
						...
						{filteredData?.map((ticket) => (
							<React.Fragment key={ticket.id}>
								<tr
									...
									onClick={() => setPreviewTicket(ticket.id)}
								>
									<td>
										<FaDotCircle
											className={`w-7 ${
									...
									<td className="text-textDefault">
										{toPersianDateShort(ticket.created_at)}
									</td>
									<td className="text-textDefault">
										{toPersianDateShort(ticket.updated_at)}
									</td>
								</tr>
								<Modal
									title={`موضوع : ${ticket.title}`}
									open={previewTicket === ticket.id ? true : false}
				...
				<Modal open={open} onClose={() => setOpen(false)} title="تیکت جدید">
	...
	return (
		<form onSubmit={submitTicketHandler}>
			<div className="w-full px-3 mb-6 md:mb-0">
				<label for="" className="text-textDefault">
					موضوع تیکت
				</label>
				<input
					type="text"
	...
	return (
		<div>
			...
			<div className="text-sm text-slate-800 pt-4 flex items-center gap-2">
				<FaDotCircle
					className={`w-7 ${ticket.status === '0' ? 'text-red-600' : 'text-green-900'}`}
				/>
