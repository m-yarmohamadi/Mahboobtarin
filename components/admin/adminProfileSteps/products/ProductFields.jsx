	...
	return (
		<form onSubmit={formik.handleSubmit} className="w-full space-y-4 py-6">
			...
			<div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
				<Input label={'نام'} formik={formik} name={'title'} required={true} />
...
function PictureSelector({ formik }) {
	const { mutateAsync: mutateUploadPhotos, isPending: isUploading } = useMutation({
		mutationFn: updloadProductPhotos,
	});
				...
				{formik.values.files.length ? (
					<div className="flex flex-wrap gap-4">
						{formik.values.files.map((file) => (
							<div
								key={file.id}
								className="w-24 h-24 rounded-lg overflow-hidden p-1 border border-slate-300 relative"
							>
								<img
									src={file?.isEdit ? file.file : URL.createObjectURL(file.file)}
									alt=""
					...
					formik?.touched?.files && (
						<div className="w-full flex justify-start items-start mt-2">
							<p className="error_Message">{formik?.errors?.files}</p>
						</div>
					)
				)}
			</div>
