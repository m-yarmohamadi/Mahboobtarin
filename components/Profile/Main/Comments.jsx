				...
				{comments.map((item) => {
					return (
						...
						>
							...
							<span className="text-slate-600 text-xs font-thin">{item.text}</span>
						</div>
					);
				})}
			</div>
			...
			<div className=" flex flex-col items-center gap-4 w-full  p-4">
				...
				<div className="flex items-center gap-3">
					{Array(5)
						...
						.map((item, index) => (
							<div
								onClick={() => formik.setFieldValue('star', index + 1)}
								key={index}
								className={`cursor-pointer ${
									index < formik.values.star ? 'text-slate-800' : 'text-slate-300'
								}`}
							>
								<FaStar />
							</div>
