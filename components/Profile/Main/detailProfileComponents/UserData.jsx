	...
	return (
		<div className="flex flex-col">
			<div className="flex items-center gap-2">
				...
				{userData?.is_verify === true && (
					<BsFillPatchCheckFill className="w-4 h-4 text-primary-01" />
				)}
			</div>
			<span className=" text-xs text-slate-700">{userData?.unique_url_id}@</span>
			<div className="flex items-center gap-1 py-2">
				{userData?.expertises?.map(
					(expertise, index) =>
						expertise.subject && (
							<div key={index} className="group text-xs md:text-sm text-primary-01">
