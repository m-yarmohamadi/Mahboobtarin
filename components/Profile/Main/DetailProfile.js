...
import { FaHeart, FaMapLocationDot, FaRegCalendar, FaRegCircleCheck } from "react-icons/fa6";
...
const DetailProfile = ({ userData, isFollow, isLike, popularList }) => {
	...
	const getProvinceLabel = !isLoading && provinces.filter((p) => Number(p.id) === Number(userData?.province_id))[0]?.name;
			...
			<div
				...
				<div className='flex justify-between gap-3 -mt-24 md:-mt-16'>
					<div className='flex flex-col justify-end items-center gap-2'>
						...
						<span className='bg-green-300 dark:bg-green-700 text-xs font-semibold text-slate-700 rounded-full py-1 px-4'>فعال</span>
					</div>
						...
						<div className='flex justify-center items-center gap-1 py-2'>
							{userData?.expertises?.map((expertise, index) => (
								<div key={index}>
									...
									<span className='bg-primary-01 rounded-sm px-1 text-xs md:text-sm text-slate-100'>{expertise.subject}</span>
								</div>
							))}
						</div>
					</div>
					...
					<div className='w-full flex flex-col gap-4'>
						...
						{userData?.addresses.length ? (
							<div className='w-full text-sm text-slate-800 flex flex-col gap-1'>
								...
								<span className='break-words'>{userData?.addresses[0].address}</span>
							</div>
						) : null}
					</div>
						...
						<>
							<div className='py-2'>
								<div className='w-full h-[200px] relative border border-primary-01 rounded-md overflow-hidden'>
									<MapView coord={[userData?.addresses[0].lat, userData?.addresses[0].lng]} />
								...
								<SwiperSlide
									...
									<div className='border border-slate-300 bg-gradient-to-b  from-blue-100 dark:from-slate-500  to-white overflow-hidden rounded-xl hover:shadow-md dark:shadow-darkMd hover:cursor-pointer'>
										...
										<div className='flex flex-col justify-center items-center gap-1 p-2'>
											...
											<span className='line-clamp-1 text-xs text-slate-500'>انتشارات:{item.Publications}</span>
										</div>
										<div className='flex justify-between p-4 items-end gap-3'>
											...
											<div className=' flex flex-col justify-start items-center'>
												<del className='text-slate-400'>{numberWithCommas(`${item.Price}`)}</del>
