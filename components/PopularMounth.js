						...
						<SwiperSlide
							...
							<div className='w-full flex justify-center items-center justify-items-center text-center px-5'>
								<div className='my-1 flex items-center justify-center rounded-full w-20 h-20 overflow-hidden ring-2 ring-primary-01 ring-offset-2'>
									<img
										className={!item?.avatar?.length ? 'w-11 h-11 md:w-12 md:h-12' : "w-full h-full object-cover object-center grayscale hover:grayscale-0"}
										src={item?.avatar?.length ? item?.avatar[0]?.path : '/images/defaultUser.png'}
										alt={`${item?.name} ${item?.lastname} `}
									/>
								</div>
							</div>
							<span className='pt-2 flex justify-center items-center'>{item.name} {item.lastname}</span>
							<span className='pb-2 flex justify-center items-center text-gray-400 text-sm'>عنوان کاربر</span>
						</SwiperSlide>
					);
				})}

				<button id='most-populars-btn-next' className='custom-button-prev z-40  -left-3'> 
					<FaChevronLeft />
				</button>
				<button id='most-populars-btn-prev' className='custom-button-next z-40  -right-3'>
					<FaChevronRight />
				</button>
			</Swiper>
