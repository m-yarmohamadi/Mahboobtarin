...
const MultiRecordeStep03 = ({recordName,recordValue ,modalName,setModalName,openModalName,setOpenModalName}) => {
	return (
		<div>
			...
			{modalName.map((item, index) => {
				const handleDeleteRecordName = (index) => {
					setModalName((prevRecordName) => prevRecordName.filter((item, i) => i !== index));
				};
				...
				return (
					<div
						...
						<div className='flex justify-center items-center gap-2 '>
							<span className='flex justify-center items-center pt-2'>{item.recordValue}</span>
							<span
