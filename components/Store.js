      ...
      <div className="w-full">
        <div className="w-full  md:flex justify-between items-center pb-4">
          <span className="font-bold text-2xl text-primary-01 flex justify-center items-center pb-8 md:pb-0">
            ...
            <span> تخفیف </span>
            &nbsp;
            <span className="text-slate-800"> در محبوب مال</span>
          </span>
          <div className="flex justify-between items-center gap-4 text-textDefault">
            <span>زمان باقیمانده:</span>

            <div className="text-lg flex justify-center items-center gap-2 bg-slate-800 py-1 px-3 text-white rounded-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          <div className="w-full bg-slate-200 hover:bg-slate-100 hover:shadow-md dark:shadow-darkMd">
            <img
          ...
          {!isLoading &&
            products?.map((item) => {
              return (
                <div
                  key={item.id}
                  className="flex flex-col overflow-hidden bg-white rounded-xl duration-300 border border-slate-300 hover:shadow-md dark:shadow-darkMd"
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <Link href={`/products/${item.id}`} className="block">
                      ...
                      <div
                        className="text-xs text-slate-600 leading-8"
                    ...
                    <div className="space-y-4">
                      <div className="w-full flex mt-4 gap-1 justify-between items-center">
                        {item.discount_price !== 0 && (
                          <span className="text-sm text-white bg-error px-4 py-2 rounded-md">
                            {item.discount_price} %
                          </span>
                        )}
                        <div className="flex flex-col justify-center items-center">
                          <p className="text-md line-through  text-slate-400">
                            {numberWithCommas(item.price)}{" "}
                          </p>
                          <p className="text-lg font-bold text-slate-700 text-left">
                            {numberWithCommas(
                              addDiscount(item.price, item.discount_price)
                            )}{" "}
                            تومان
                          </p>
                        </div>
